...
localTimeToUtcJsDate localTime = do
  ...
  let Tuple _ inputTimeUtc = Time.adjust (Duration.Minutes offsetVal) localTime
  let myDateTime = DateTime date inputTimeUtc
...
jsDateToTime jsdate = do
  ...
  case Tuple (toEnum (round hr)) (toEnum (round min)) of
    Tuple (Just hrTyped) (Just minTyped) ->
      pure $ Time hrTyped minTyped bottom bottom
    _ ->
      pure $ Time bottom bottom bottom bottom


-- infixr 6 type Tuple as /
-- type S = SProxy

-- type Locale = SProxy "UTC" / S "Local"
-- foreign import kind Locale
-- foreign import data Local :: Locale
-- foreign import data UTC :: Locale

-- type Time' (locale :: Locale) = Time
